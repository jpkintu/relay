...
3. **Apply security.** Sign in as the owner → Settings → **Apply security
   ...
   Re-run it after every Cloud Code update that adds fields (e.g. the cash
...
- [ ] **S5 P1 — A 4-digit PIN can be brute-forced.** (Server setting; see §2.) Enable Parse `accountLockout`
      ...
      numeric only.
- [x] **S6 P1 — No PIN re-entry** for handover, profile changes or shift end.
      _Done 2026-09-26: `verifyPin` (lib/core.js) re-checks the PIN on the server for
      handover, shift end and till payouts; 5 wrong PINs lock those steps for 15 min.
      Still to add when PIN change exists (Phase 3)._
...
- [x] **B4 P1 — Handover confirm and create are not atomic.** _Done 2026-09-26:
      each order is claimed for a handover with an atomic counter (all or none),
      writes are ordered so a retry is safe, `requestId` makes a resend return the
      same handover, and the nightly cash check flags anything inconsistent._ Orders and the
...
- [x] **B5 P1 — Concurrency.** _Done 2026-09-26: one reviewer per handover round
      (atomic claim), one payout per rider at a time, one cashier per kitchen order._
      Two cashiers can confirm the same handover at the
...
- [x] S6 PIN re-entry (`verifyPin`) for handover submit, shift end and till payouts (profile/PIN change comes with Phase 3)
- [x] B4 / B5 atomic, idempotent `createHandover` / `confirmHandover` / `disputeHandover`
- [x] Partial confirmation: the cashier ticks the orders that are physically covered; the rest go back to `WITH_RIDER` for the rider to hand over again
...
- [x] One cashier per kitchen order: the first cashier to act takes it, others see who has it; "Pass to a colleague" transfers it to a cashier on shift or releases it; a cashier cannot end their shift while holding orders — owner's decision 2026-09-26
...
6. ~~One cashier device or several?~~ Answered 2026-09-26: **several at once**; each order belongs to one cashier, who can pass it to a colleague on shift.
...
| 2026-09-26 | README with video guides for riders, cashiers and the owner (`docs/videos`: MP4 walkthroughs with on-screen step captions, plus thumbnails).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-09-26 | Cash integrity (batch 1): customers pay the full total; rider pay = commission + delivery fee, paid from the till (`payRider`, PIN) with other till expenses (`recordTillPayout`); expected till = opening + cash in − payouts. One cashier per kitchen order with transfer/release (`transferOrder`, `getOnShiftCashiers`); a cashier cannot end a shift holding orders. Race-safe, idempotent handovers; partial acceptance returns unticked orders to the rider; owner resolves disputes (deduct from pay, write off, reopen) and can take cash in person. PIN re-entry with lockout for handover, shift end and payouts. Stale-handover (> 4 h) alerts; nightly/on-demand cash check. New cashier Payouts tab, rider handover list and "Owed to you". e2e 82. |
| 2026-09-26 | Fix: the cashier handover count dialog lost its order lines (ticked total dropped to 0) when the list refreshed every 10 s; loaded lines are now kept.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-09-26 | Rider pay = commission + delivery fee everywhere: payouts, earnings, commission ledger and reports (revenue after rider pay takes the delivery fees off too). Deliveries recorded before the fee was included are paid it, and Apply security rules repairs them. The cashier can pay the rider for the handed-over orders when confirming the handover (PIN; recorded as a till payout). Payouts page redesigned to match the other tables (totals, commission / delivery fee split); stacked phone tables show their totals on one line. e2e 85.                                                                                                                                                                                                                |
| 2026-09-26 | Mobile money taken at the door must be confirmed: it stays on the rider's list ("waiting for the cashier") and blocks shift end until confirmed; if the cashier rejects it, the order becomes cash the rider owes (back on their cash list, counts toward the cash limit) until they hand over the cash or send a correct transaction ID. Delivery screens no longer say "the kitchen starts…" for these payments. e2e 87.                                                                                                                                                                                                                                                                                                                                        |
