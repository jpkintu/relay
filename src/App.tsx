...
export default function App() {
  ...
  useEffect(() => {
    ...
    query.find().then((roles) => {
      ...
      setRole(names.includes("admin") ? "admin" : names.includes("cashier") ? "cashier" : "rider");
    }).catch(() => {setRole("rider");setCanInitialize(true)});
  }, [user?.id]);
  ...
  return (
    <div className={preview ? "preview-app" : undefined}>
      {workspace}
      {user && canInitialize && !preview && <div className="owner-initialize"><span>First owner account? Set up administrator access.</span><button onClick={async()=>{try{await Parse.Cloud.run('bootstrapOwner');setCanInitialize(false);setRole('admin');setSetupError('')}catch(e){setSetupError(e instanceof Error?e.message:'Unable to initialize owner')}}}>Initialize owner</button>{setupError&&<small>{setupError}</small>}</div>}
        <nav className="role-switch" aria-label="Preview role navigation">
          <strong>{preview ? "LIVE PREVIEW" : "RELAY"}</strong>
          {preview && (["rider", "cashier", "admin"] as Role[]).map((item) => (
            <button
          ...
          <button className="preview-login" onClick={() => {setPreview(p=>!p);setRole('rider')}}>{preview ? "Sign in" : "Preview roles"}</button>
        </nav>
    </div>
  );
}
