    ...
    try {
      ...
      setTickets(
        rows.map((row) => ({
          ...
          rider: row.get('createdBy')?.id || 'Rider',
          customer: row.get('customerName'),
          items: 'Items recorded on ticket',
          total: row.get('total'),
          status: row.get('status'),
          stage:
            row.get('status') === 'PLACED'
              ? 'Incoming'
  ...
  const move = async (ticket: Ticket, stage: Ticket['stage'] | null) => {
    ...
    setTickets((p) =>
      stage
        ? p.map((t) =>
            t.code === ticket.code
              ? { ...t, stage, status: stage === 'Preparing' ? 'ACCEPTED' : stage.toUpperCase() }
              : t,
          )
    ...
    <main className="ops-shell">
      <header className="ops-header">
        ...
        <nav>
          ...
          <button className={tab === 'shift' ? 'active' : ''} onClick={() => setTab('shift')}>
            <Clock3 />
            Shift
          </button>
        </nav>
